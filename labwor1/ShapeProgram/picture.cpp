...
namespace shapes 
{
    ...
    void Picture::ChangeShape(const std::string& id, std::shared_ptr<Shape> newShape) // применить стратегию для фигур, замена фигуры не должна происходить при её изменении
    {
    ...
    void Picture::MoveBy(double dx, double dy)
    {
        for (auto& shape : shapes) 
        {
            shape->MoveBy(dx, dy);
